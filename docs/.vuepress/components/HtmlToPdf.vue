<template>
  <div class="html2pdf">
    <div class="btn tap-active" @click="onClick">
      <svg
        class="iconfont"
...
export default {
  ...
  methods: {
    async onClick() {
      const canvas = await html2canvas(document.querySelector('.theme-default-content'), defaultOptions)
      const contentWidth = canvas.width
      const contentHeight = canvas.height
      const pageHeight = Math.ceil(contentWidth / 592.28 * 841.89)
