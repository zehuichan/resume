...
export const resume: Resume = {
  profile: {
    ...
    meta: ['广州', '本科', '随时到岗'],
    summary:
      '10 年+ 前端研发，从一个人扛整站，到给团队做组件库，再到现在做集团级前端基建。敬城后台、H5、小程序并行交付，我把模板、组件、脚手架、接口标准和监控告警一次立齐，新仓开箱即用，存量 ==21== 个业务前端仓库统一到同一套。现任敬城集团前端负责人，主导==前端基建主链路==。',
    contacts: [
      {
        icon: Mail,
        label: 'Email',
    ...
    timeline: [
      {
        year: '2015',
        head: '从业务实现到整站负责',
        body: '在佛山电子口岸从做页面到接整站，技术选型、工程搭建与疑难排查开始由自己拍板。'
      },
      {
        year: '2020',
        head: '从项目交付到资产沉淀',
        body: '在中视信息把高频业务控件整理成公司级组件库，发到私服，重复的需求不用每个项目再写一遍。'
      },
      {
        year: '2022',
        head: '从个人效率到团队治理',
        body: '在兴工科技做前端主程，把分支策略、Code Review 和 CI/CD 立起来，问题尽量在提交阶段就拦下。'
      },
      {
        year: '2025',
        head: '从规范治理到基建主链路',
        body: '在敬城集团建成模板、组件、脚手架、接口标准、Agent Skills 与监控告警一体的前端基建，建仓、写码、回归、上线观测走同一套。'
      }
    ],
    projects: [
      {
        ...
        role: '体系主导',
        featured: true,
        scene: '集团后台、H5、小程序一起开，各仓模板、组件、接口和测试各搞一套。新仓反复从零搭，老仓没法统一回灌。',
        action:
          '把规范直接做成能装的脚手架：建仓即带三端模板、brick 组件和回归层；接口以 Apifox 为唯一真相源统一生成类型与请求；Sentry SOP 全量接入业务系统，Issue 进企微告警群。',
        result:
      ...
      },
      {
        ...
        role: '独立主导',
        featured: true,
        scene: '团队规模化用 `Cursor`、`Claude Code` 写 CRUD 和组件，目录、写法、自测和评审全凭个人习惯，生成结果合不进生产。',
        action:
          '把提示词做成可安装、可打版本的 Skills：`vue-vben-crud` 管 CRUD 骨架，组件实践、UI 自测、第一性原理评审各占一个；E2E SOP 随 `create-e2e` 下发，回归统一走 Playwright。',
        result: '标准 CRUD 由 Agent ==分钟级生成==；评审与 UI 自测进日常流程，产出直接落到脚手架自带的回归层。'
      },
      {
        name: '和林国际物流信息管理系统',
        kind: '国际物流全链路',
      ...
      },
      {
        ...
        role: '核心开发',
        scene: '医药电商 H5 商城，药品、保健、医疗器械、医生预约到下单支付，全塞在同一套移动端壳里。',
        action:
          '统一 `axios` 请求和异常兜底，多机型适配交给 postcss-px-to-viewport 收口；地址、预约时间、优惠券这些重复表单抽成业务组件。',
        result: '交付商品浏览、购物车、下单支付、医生预约与积分卡券==全链路页面==，请求层和业务组件一直沿用到后面几个版本。'
      }
    ]
  },
    ...
    projects: [
      {
        ...
        role: '独立开发',
        sortDate: '2020.07',
        scene: '法律咨询移动端 App，内容课程、在线咨询、支付、订单和钱包，一个人从零做到上线。',
        action: '前端搭建和接口联调都是自己做；在线咨询接 `tim-js-sdk`，分享配置、图片上传抽成公共能力。',
      ...
      },
      {
        ...
        role: '核心开发',
        sortDate: '2019.03',
        scene: '燃气用户、客服与施工师傅共用一套公众号加后台，从缴费报数到安装、改管、维修、安检都要线上流转。',
        action: '负责前站与配套后台主要功能；用全局 filters 与 `require.context` 自动注册 vuex module，压掉多模块的重复接线。',
      ...
      },
      {
        ...
        role: '核心开发',
        sortDate: '2018.07',
        scene: '「单一窗口」延伸的口岸信息化平台，查验管理、集装箱动态与多类预警都要按机构和角色分权。',
        action: '用 `router.beforeEach` 配合 addRoutes 做动态菜单，再以 ==v-permission== 指令把按钮级校验统一到一处。',
        result: '菜单与按钮双层权限支撑机构、角色分级管理，查验与多类预警模块长期稳定运行。'
      }
    ]
  },
...
'业余时间独立维护 `workbench`、`element-components`、`vant-components` 三个库，分别是可编辑表格工作台，以及 PC 端和移动端的业务组件库。GitHub 公开仓库 **8 个**。',
    ...
    highlightTag: '资产沉淀',
    highlight:
      '个人组件库、workbench，到后来集团的 brick 三端组件和 Agent Skills，其实是同一件事：把踩过的坑做成别人装上就能用、还能自查的东西。'
  },

  skills: [
    {
      label: '跨端研发',
      body: '物流、科技成果、ETC、商城、燃气这些项目里，用 `vue2/3`、`taro`、`uniapp` 和原生小程序交付过 H5、公众号、小程序和后台，多端并行时能压在同一套工程规范上。'
    },
    {
      label: '架构与组件',
      body: '做了三端标准模板和 brick / brick-mobile / brick-mini 三端组件（Ant Design Vue / Vant / TDesign UniApp）；Tenon 的 Schema 与物料分层也拆成了能单独发布的包。'
    },
    {
      label: '健壮性工程',
      body: '请求层、权限、字典和异常兜底都是先定好再写业务；Sentry SOP 已全量接入公司业务系统，Issue 进企微告警群推送；业务仓再用 `create-e2e` 接 Playwright 回归和 agent-browser 探路写测。'
    },
    {
      label: '工程化协作',
      body: '主导 `vite` / monorepo 与 GitLab CI/CD；`create-app@1.0.0`、`create-e2e@1.2.0` 已发私服，工作区内 ==21== 个业务前端仓统一靠它们建仓、接回归。'
    },
    {
      label: 'AI 工程化',
      body: '维护 `agent-skills`：`vue-vben-crud`、`vue-components-practices`、`ui-autotest`、`first-principles-review`；`e2e-module-sop` 跟着 create-e2e 下发，Agent 按团队标准产码和写测。'
    },
    {
      label: '接口与联调',
      body: '公司 OpenAPI 约定以 Apifox 为真相源，`openapi-ts-request` + `pnpm gen:api` 统一 `generated/` 目录、请求适配与 `code/message/result` 错误码语义。'
    }
  ],
